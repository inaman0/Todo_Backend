...
public String generateJson(@RequestBody WrapperClass wrapperClasses, @RequestParam String appName) throws Exception {
     ...
     p.generator(appPath,project);
        if (enumDtos != null && !enumDtos.isEmpty()) {
            ...
            javaCode.append("}\n}\n}");
                ...
                String outputPath = appPath+"/src/main/java/com/flightBooking/";
    ...
    for(MetaDataDto dto:dtos) {
        ...
        javaCode.append("  \"cluster\" : \"traveler_db\",\n");
...
//       }
      //  String outputPath = "C:\\repos\\rasp-iiitb360\\src\\main\\java\\org\\rasp\\iiitb720";
        ...
        String controller_dir = appPath+"src/main/java/controller";
        String controller_package = "controller";
        ResourceGenerator generator = new ResourceGenerator("com.flightBooking", appPath+"src/main/java/com/flightBooking/");
        ...
        generator.generateCode(appPath+"src/main/java/com/flightBooking/", null);
        if (enumDtos != null && !enumDtos.isEmpty()){
            for (EnumDto dto:enumDtos) {
                javaCode = new StringBuilder();
                javaCode.append("package controller;\n");
                javaCode.append("import com.flightBooking.resource"+"."+toPascalCase(dto.getEnum_name())+";\n");//restrict user to send camel case
...
javaCode.append(" for(Field f:fields){\n   if(f.getName().startsWith(\"ID\")){\ncontinue;\n}\n Object value = f.get(null);\n names.add(value); \n }\n return names;\n");
        ...
        javaCode.append("}\n}");
                ...
                String outputPath = appPath+"/src/main/java/controller";
                // Create the directory structure if it doesn't exist
                File directory = new File(outputPath);
                if (!directory.exists()) {
                    directory.mkdirs();
